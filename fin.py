...
def _seed_cases(conn):
    ...
    specials = conn.execute("SELECT id FROM gifts WHERE category='special'").fetchall()
    for sp in specials:
        conn.execute("INSERT INTO case_items (case_id,gift_id,weight) VALUES (?,?,?)", (rare_id, sp["id"], 2))
    for i in range(9):
        conn.execute("INSERT INTO case_items (case_id,gift_id,weight) VALUES (?,NULL,?)", (rare_id, 10))
    ...
    conn.execute(
        "INSERT INTO cases (name,emoji,price_stars,description,bear_chance) VALUES (?,?,?,?,?)",
        ("Микс", "🎲🎁", 50, "Смесь подарков! 75% на подарки до 50⭐, 25% на подарки от 55⭐", 0)
    )
    mix_id = conn.execute("SELECT last_insert_rowid()").fetchone()[0]
    
    cheap = conn.execute("SELECT id FROM gifts WHERE price_stars <= 50 AND price_stars IS NOT NULL AND category='regular'").fetchall()
    cheap_weight = 75 // len(cheap) if cheap else 75
    for c in cheap:
        conn.execute("INSERT INTO case_items (case_id,gift_id,weight) VALUES (?,?,?)", (mix_id, c["id"], cheap_weight))
    
    expensive = conn.execute("SELECT id FROM gifts WHERE price_stars >= 55 AND price_stars IS NOT NULL").fetchall()
    exp_weight = 25 // len(expensive) if expensive else 25
    for e in expensive:
        conn.execute("INSERT INTO case_items (case_id,gift_id,weight) VALUES (?,?,?)", (mix_id, e["id"], exp_weight))
...
def add_to_inventory(uid, gift_id, gift_name, gift_emoji, gift_price):
    ...
    conn.execute(
        ...
        (uid, gift_id, gift_name, gift_emoji, gift_price)
    )
    conn.commit()
    conn.close()
    log.info(f"Added to inventory: user {uid}, gift {gift_name}")
...
def _deliver_gift(uid, gift, chat_id, silent=False):
    ...
    add_to_inventory(uid, gift["id"], gift["name"], gift["emoji"], gift.get("price_stars") or 0)
...
def purchase_gift_balance(chat_id, uid, gift_id):
    ...
    if _deliver_gift(uid, dict(gift), chat_id, silent=True):
        send(chat_id, f"✅ {gift['emoji']} {gift['name']} добавлен в инвентарь!", inline([
...
def show_profile(chat_id, uid, msg_id=None):
    u = get_user(uid)
    if not u:
        return
    ...
    kb = [
        [("💳 Пополнить", "topup")],
        [("🎒 Инвентарь", "inventory")],
        [("👥 Рефералы", f"referral_{uid}")],
    ]
    if pending > 0:
        kb.insert(2, [(f"🎁 Забрать награду ({pending})", f"claim_rewards_{uid}")])
    kb.append([(f"Уведомления: {notif_label}", "toggle_notif")])
    kb.append([("◀️ В меню", "main_menu")])
    
    if msg_id:
        edit_msg(chat_id, msg_id, text, inline(kb))
    else:
        send(chat_id, text, inline(kb))


def show_referral_menu(chat_id, uid, msg_id=None):
    u = get_user(uid)
    if not u:
        return
    
    conn = db_connect()
    invited_count = db_scalar(conn, "SELECT COUNT(*) FROM users WHERE referrer_id=?", (uid,)) or 0
    total_spent_by_refs = db_scalar(conn, "SELECT SUM(total_spent) FROM users WHERE referrer_id=?", (uid,)) or 0
    pending = get_pending_rewards_count(uid)
    conn.close()
    ...
    text = (f"👥 <b>Реферальная система</b>\n\n"
            ...
            f"💰 Потрачено рефералами: {total_spent_by_refs:.0f}⭐\n"
            f"🎁 Доступно наград: {pending}\n\n"
            f"<i>За каждые 25⭐, потраченные приглашённым, ты получаешь награду!</i>")
    
    kb = []
    if pending > 0:
        kb.append([("🎁 Забрать награды", f"claim_rewards_{uid}")])
    kb.append([("◀️ Назад", "profile")])
...
def show_inventory(chat_id, uid, msg_id=None):
    conn = db_connect()
    items = conn.execute(
        "SELECT id, gift_id, gift_name, gift_emoji, gift_price FROM user_inventory WHERE user_id=? ORDER BY obtained_at DESC", (uid,)
    ).fetchall()
    conn.close()
    
    if not items:
        if msg_id:
            edit_msg(chat_id, msg_id, "🎒 <b>Инвентарь пуст</b>\n\nПокупай подарки или открывай кейсы!", inline([[("◀️ Назад", "profile")]]))
        else:
            send(chat_id, "🎒 <b>Инвентарь пуст</b>\n\nПокупай подарки или открывай кейсы!", inline([[("◀️ Назад", "profile")]]))
        return
    
    set_user_state(uid, "inventory_view", data={"items": [dict(i) for i in items], "page": 0})
    _show_inventory_page(chat_id, uid, 0, msg_id)


def _show_inventory_page(chat_id, uid, page, msg_id=None):
    state = get_user_state(uid)
    if not state or state["action"] != "inventory_view":
        return
    ...
    item = items[page]
    sell_price = int(item["gift_price"] * 0.9)
    text = (f"🎒 <b>Инвентарь</b> ({page+1}/{len(items)})\n\n"
            f"{item['gift_emoji']} <b>{item['gift_name']}</b>\n"
            f"💰 Цена: {item['gift_price']}⭐\n"
            f"💸 Продажа: {sell_price}⭐")
    
    kb_buttons = []
    if page > 0:
        kb_buttons.append(("◀️ Назад", f"inv_page_{page-1}"))
    if page < len(items) - 1:
        kb_buttons.append(("Вперёд ▶️", f"inv_page_{page+1}"))
    
    kb = []
    if kb_buttons:
        kb.append(kb_buttons)
    kb.append([(f"💰 Продать за {sell_price}⭐", f"sell_inv_{item['id']}")])
    kb.append([("📤 Вывести подарок", f"withdraw_inv_{item['id']}")])
    kb.append([("◀️ Назад", "profile")])
...
def show_admin_stats(chat_id):
    ...
    total_inv     = db_scalar(conn, "SELECT COUNT(*) FROM user_inventory") or 0
    conn.close()
    send(chat_id,
         f"📊 Статистика\n\n"
         ...
         f"🎒 Предметов в инвентаре: {total_inv}",
...
def _activate_check(uid, uname, chat, code):
    ...
    try:
        ...
        input_password = None
        if ":" in code:
            real_code, input_password = code.split(":", 1)
        
        conn = db_connect()
        check = conn.execute("SELECT * FROM checks WHERE code=?", (real_code,)).fetchone()
        if not check:
            conn.close()
        ...
        if ":" in check["code"]:
            stored_password = check["code"].split(":", 1)[1]
            if not input_password or input_password != stored_password:
                conn.close()
                send(chat, f"❌ Неверный пароль!\nЧек защищён паролем.")
                return
        ...
        add_to_inventory(uid, gift["id"], gift["name"], gift["emoji"], gift["price_stars"])
        ...
        if ok:
            add_prize(uid)
            send(chat, f"🎉 Чек активирован!\n{gift['emoji']} <b>{gift['name']}</b> добавлен в инвентарь и отправлен!")
            sender_u = get_user(sender_id)
            if sender_u:
                send(sender_id, f"✅ Чек <code>{real_code}</code> активировал {user_label}\n🎁 {gift['emoji']} {gift['name']}\n📊 Осталось: {remaining}")
        else:
            conn3 = db_connect()
...
def _finalize_check(uid, chat, data):
    ...
    elif check_type == "private":
        text = (f"🧾 <b>Приватный чек от {sender_name}</b>\n\n"
                f"{gift['emoji']} {gift['name']}\n"
                f"💰 {gift['price_stars']}⭐ × {max_act} = {total_cost}⭐\n"
                f"🔐 Пароль: <code>{password}</code>\n"
                f"🔑 Код: <code>{code}</code>")
        send(chat, f"✅ Приватный чек создан!\n\n{text}", kb_url)
    else:
...
def handle_inline_query(iq):
    ...
    if not parts:
        ...
        results = [{
            ...
            "description": f"@{BOT_USERNAME} {g['id']} <кол-во> [пароль] [@username]",
            "input_message_content": {"message_text": f"@{BOT_USERNAME} {g['id']} <кол-во> [пароль] [@username]"},
        } for g in gifts]
    ...
    if len(parts) > idx and not parts[idx].startswith("@"):
        password = parts[idx]
        idx += 1
    
    if len(parts) > idx and parts[idx].startswith("@"):
        recipient = parts[idx].lstrip("@")
    
    check_type = "private" if password else "public"
    if recipient:
        check_type = "personal"
    ...
    if check_type == "personal":
        title = f"👤 Личный чек для @{recipient}: {gift['emoji']} {gift['name']}"
        descr = f"{gift['price_stars']}⭐ · {max_act} активаций · только для @{recipient}"
        if password:
            descr += f" · пароль: {password}"
        msg_text = (f"🧾 <b>Личный чек для @{recipient} от {sender_name}</b>\n\n"
                    f"{gift['emoji']} <b>{gift['name']}</b>\n"
                    f"💰 {gift['price_stars']}⭐ · 🔢 активаций: {max_act}\n"
                    f"🕒 {datetime.now().strftime('%d.%m.%Y %H:%M')}\n")
        if password:
            msg_text += f"🔐 Пароль: <code>{password}</code>\n"
        msg_text += f"\n👇 Нажми кнопку чтобы забрать!"
    elif check_type == "private":
        title = f"🔒 Приватный чек: {gift['emoji']} {gift['name']} ×{max_act}"
        descr = f"{gift['price_stars']}⭐ · {max_act} активаций · требуется пароль: {password}"
        msg_text = (f"🧾 <b>Приватный чек от {sender_name}</b>\n\n"
                    f"{gift['emoji']} <b>{gift['name']}</b>\n"
                    f"💰 {gift['price_stars']}⭐ каждому · 🔢 активаций: {max_act}\n"
                    f"🔐 Пароль: <code>{password}</code>\n"
                    f"🕒 {datetime.now().strftime('%d.%m.%Y %H:%M')}\n\n"
                    f"👇 Нажми кнопку и введи пароль!")
    else:
        title = f"🎁 Публичный чек: {gift['emoji']} {gift['name']} ×{max_act}"
        descr = f"{gift['price_stars']}⭐ каждому · {max_act} активаций"
    ...
    if check_type == "personal" and recipient:
        conn3 = db_connect()
        rec = conn3.execute("SELECT user_id FROM users WHERE username=?", (recipient,)).fetchone()
        conn3.close()
        if rec:
            send(rec["user_id"], f"🎁 Тебе прислали личный чек!\n\n{msg_text}", kb_url)
...
def handle_user_state(uid, uname, chat, text, state):
    ...
    if action == "create_check":
        ...
        elif step == 2 and data.get("check_type") == "private":
            try:
                max_act = int(text.strip())
                if not (1 <= max_act <= 100):
                    raise ValueError
    ...
    if action == "inventory_view":
        if text.startswith("/") or text == "◀️ В меню":
            clear_user_state(uid)
            show_main_menu(chat)
        return
...
def handle_callback(cb):
    ...
    if data == "main_menu":
        ...
        show_profile(chat, uid, msg_id)

    elif data == "inventory":
        show_inventory(chat, uid, msg_id)

    elif data.startswith("referral_"):
        referrer_id = int(data.split("_")[1])
        if uid != referrer_id:
            send(chat, "❌ Это не твой профиль")
            return
        show_referral_menu(chat, uid, msg_id)

    elif data.startswith("inv_page_"):
        page = int(data.split("_")[-1])
        _show_inventory_page(chat, uid, page, msg_id)

    elif data.startswith("sell_inv_"):
        item_id = int(data.split("_")[-1])
        conn = db_connect()
        item = conn.execute("SELECT * FROM user_inventory WHERE id=? AND user_id=?", (item_id, uid)).fetchone()
        if not item:
            conn.close()
            edit_msg(chat, msg_id, "❌ Подарок не найден", inline([[("◀️ Назад", "profile")]]))
            return
        sell_price = int(item["gift_price"] * 0.9)
        update_balance(uid, sell_price)
        add_transaction(uid, "inventory_sell", sell_price, f"Продажа: {item['gift_name']}")
        conn.execute("DELETE FROM user_inventory WHERE id=?", (item_id,))
        conn.commit()
        conn.close()
        send(chat, f"✅ {item['gift_emoji']} {item['gift_name']} продан за {sell_price}⭐!")
        show_inventory(chat, uid, msg_id)

    elif data.startswith("withdraw_inv_"):
        item_id = int(data.split("_")[-1])
        conn = db_connect()
        item = conn.execute("SELECT * FROM user_inventory WHERE id=? AND user_id=?", (item_id, uid)).fetchone()
        if not item:
            ...
            return
        conn.close()
        gift = {"id": item["gift_id"], "name": item["gift_name"], "emoji": item["gift_emoji"], "price_stars": item["gift_price"]}
        if _deliver_gift(uid, gift, chat, silent=False):
            conn2 = db_connect()
            conn2.execute("DELETE FROM user_inventory WHERE id=?", (item_id,))
            conn2.commit()
            conn2.close()
            send(chat, f"✅ {item['gift_emoji']} {item['gift_name']} выведен!")
        show_inventory(chat, uid, msg_id)
        ...
        send(chat,
             ...
             inline([
                 ...
                 [("🔒 Приватный", f"check_type_private_{gift_id}")],
    ...
    elif data.startswith("check_type_private_"):
        gift_id = int(data.split("_")[-1])
        set_user_state(uid, "create_check", 1, {"gift_id": gift_id, "check_type": "private"})
        send(chat, "🔐 Введите пароль для чека:")
